# -*- coding: utf-8 -*-
"""Research-only 09:30-10:30 opening-core 60s audit.

This route trains a dedicated V4R-style two-head model on OPEN_CORE rows only.
Validation selects head thresholds using opening rows only; frozen TEST is touched
once. Promotion still compares against the same whole-day V4R benchmark and uses
coverage relative to all frozen test rows, so a tiny opening-only toy cannot pass.
Production V18 is never changed here.
"""
...
VERSION = "opening-core-audit-v1-20260907"
CLOUD_SCOPE = "QMT_L1_60S_OPENING_CORE_V1"
HURDLE_BP = 2.0
OUT_ROOT = Path.home() / "AStockData" / "opening_core_v1"
OPEN_LO = 570.0
OPEN_HI = 630.0
...
def _opening(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    return frame[(minute >= OPEN_LO) & (minute < OPEN_HI)].copy()


def _pred(up_model: Any, dn_model: Any, frame: pd.DataFrame, up_t: float, dn_t: float) -> np.ndarray:
    up = core._positive_probability(up_model, frame[core.FEATURES])
    dn = core._positive_probability(dn_model, frame[core.FEATURES])
    return core._combine(up, dn, up_t, dn_t)


def _metrics(frame: pd.DataFrame, pred: np.ndarray, full_test_rows: int) -> Dict[str, Any]:
    ...
    ex = exact[active & np.isfinite(exact)]
    return {
        "test_rows": int(full_test_rows),
        "opening_rows": int(len(frame)),
        "directional_predictions": n,
        "directional_coverage_pct": (100.0 * n / full_test_rows) if full_test_rows else None,
        "opening_signal_rate_pct": (100.0 * n / len(frame)) if len(frame) else None,
        ...
        "up_predictions": int(((pred == 1) & active).sum()),
        "down_predictions": int(((pred == -1) & active).sum()),
    }
...
def _aggregate(folds: List[Dict[str, Any]], key: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "daily": [dict(day=f["test_day"], **f[key]) for f in folds],
    }
...
def _sync(report: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        payload = {
            ...
            "protocol": "OPEN_CORE_0930_1030_DEDICATED_TWO_HEAD__VALIDATION_THRESHOLDS__FROZEN_TEST__2BP",
...
def run(dataset_root: Path, dates: List[str]) -> Dict[str, Any]:
    ...
    try:
        for fold_no, idx in enumerate(test_indices, 1):
            ...
            tr, va, te, _ = splitbase._split(frame)
            tr_o = _opening(tr); va_o = _opening(splitbase._nonoverlap(va)); te_all = splitbase._nonoverlap(te); te_o = _opening(te_all)
            train_fit = v4r._rotating_thin(tr_o, core.TRAIN_THIN_SECONDS)
            if train_fit.empty or va_o.empty or te_o.empty:
                raise RuntimeError(f"opening split empty fold {fold_no}")
            models = core._models()
            up = models["logistic_balanced"]; dn = core._models()["logistic_balanced"]
            core._fit("logistic_balanced", up, train_fit[core.FEATURES], train_fit[core.UP_TARGET].astype(int).to_numpy())
            core._fit("logistic_balanced", dn, train_fit[core.FEATURES], train_fit[core.DOWN_TARGET].astype(int).to_numpy())
            up_t, _, _ = core._choose_head(up, va_o, 1, HURDLE_BP, core.UP_TARGET)
            dn_t, _, _ = core._choose_head(dn, va_o, -1, HURDLE_BP, core.DOWN_TARGET)
            pred = _pred(up, dn, te_o, up_t, dn_t)
            folds.append({
                "fold": fold_no, "test_day": test_day,
                "up_threshold": up_t, "down_threshold": dn_t,
                "candidate": _metrics(te_o, pred, len(te_all)),
                "test_used_for_threshold_selection": False,
            })
    finally:
        v3_mod._expand = original_expand

    candidate = _aggregate(folds, "candidate")
    # Same benchmark values from the identical historical V3 protocol.
    v3_report = json.loads((base.OUT_ROOT.parent / "qmt_l1_walkforward_v3" / "latest.json").read_text(encoding="utf-8"))
    baseline = (((v3_report.get("aggregates") or {}).get("V4R") or {}).get("logistic_balanced") or {})
    baseline = {k: baseline.get(k) for k in (
        "directional_predictions", "directional_coverage_pct", "directional_accuracy_pct",
        "accuracy_wilson_95_lower_pct", "avg_net_edge_bp", "positive_net_edge_days", "test_rows"
    )}
    return {
        ...
        "opening_window": "09:30-10:30", "dedicated_opening_model": True,
        "validation_only_threshold_selection": True, "test_used_for_threshold_selection": False,
...
def main() -> int:
    ...
    print(f"[OPEN] acc={c.get('directional_accuracy_pct')} cov={c.get('directional_coverage_pct')} net={c.get('avg_net_edge_bp')} n={c.get('directional_predictions')} gate={report['development_gate']['pass']}")
