...
echo V3 + iMACD predictor audit + V4R x iMACD filter audit
...
"%PYEXE%" -m py_compile "%SERVICEDIR%\qmt_l1_60s_walkforward_v3.py" "%SERVICEDIR%\imacd_research_audit_v1.py" "%SERVICEDIR%\run_imacd_audit_v1.py" "%SERVICEDIR%\v4r_imacd_filter_audit_v1.py"
...
if "%FRESHV3%"=="1" (
  echo [3/5] Reusing the fresh V3 Tick dataset already on this PC.
) else (
  echo [3/5] No fresh V3 dataset found; running 10-session QMT Tick replay first...
...
echo Filter family is predeclared; validation chooses; test never tunes filter rules.
...
if "%RC%"=="0" (
  ...
  echo [RULE] A filter is an upgrade only if Accuracy AND Net Edge beat V4R and all hard gates pass.
  echo [RULE] Historical pass enters research/shadow only; production still requires unseen prospective days.
) else (
  echo [ERROR] V4R x iMACD filter audit failed with rc=%RC%.
