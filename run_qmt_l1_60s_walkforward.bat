...
echo V3 + iMACD + state filter + rolling-OOF meta-label + cross-rank + direction asymmetry
...
if "%FRESHV3%"=="1" (
  echo [3/8] Reusing the fresh V3 Tick dataset already on this PC.
) else (
  echo [3/8] No fresh V3 dataset found; running 10-session QMT Tick replay first...
...
echo [4/8] Running fixed iMACD predictor audit for reference...
set "ASTOCK_SKIP_RESEARCH_CHAIN=1"
"%PYEXE%" -u -m services.run_imacd_audit_v1
set "ASTOCK_SKIP_RESEARCH_CHAIN="
if errorlevel 1 goto :runfail

echo [5/8] Running V4R x iMACD validation-only regime-filter audit...
...
echo [6/8] Running V4R rolling-OOF meta-label audit V2...
...
echo [7/8] Running fixed-universe cross-sectional ranking audit...
...
echo [8/8] Running validation-only direction-asymmetry audit...
...
if "%RC%"=="0" (
  ...
  echo [ERROR] Direction-asymmetry audit failed with rc=%RC%.
