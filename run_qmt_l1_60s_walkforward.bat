...
echo V3 + iMACD + V4R state filter + V4R meta-label audit
...
"%PYEXE%" -m py_compile "%SERVICEDIR%\qmt_l1_60s_walkforward_v3.py" "%SERVICEDIR%\imacd_research_audit_v1.py" "%SERVICEDIR%\run_imacd_audit_v1.py" "%SERVICEDIR%\v4r_imacd_filter_audit_v1.py" "%SERVICEDIR%\v4r_meta_label_audit_v1.py"
...
if "%FRESHV3%"=="1" (
  echo [3/6] Reusing the fresh V3 Tick dataset already on this PC.
) else (
  echo [3/6] No fresh V3 dataset found; running 10-session QMT Tick replay first...
...
echo [4/6] Running fixed iMACD predictor audit for reference...
set "ASTOCK_SKIP_RESEARCH_CHAIN=1"
"%PYEXE%" -u -m services.run_imacd_audit_v1
set "ASTOCK_SKIP_RESEARCH_CHAIN="
if errorlevel 1 goto :runfail

echo [5/6] Running V4R x iMACD validation-only regime-filter audit...
...
echo [6/6] Running V4R validation-split meta-label audit...
...
if "%RC%"=="0" (
  ...
  echo [ERROR] Meta-label audit failed with rc=%RC%.
