...
echo V3 + iMACD + state filter + rolling-OOF meta-label + cross-rank + direction asymmetry + opening-core
...
findstr /C:"v4r-direction-asymmetry-audit-v1-20260906" "%SERVICEDIR%\v4r_direction_asymmetry_audit_v1.py" >nul || goto :fail
findstr /C:"opening-core-audit-v1-20260907" "%SERVICEDIR%\opening_core_audit_v1.py" >nul || goto :fail

echo [2/9] Syntax check...
"%PYEXE%" -m py_compile "%SERVICEDIR%\qmt_l1_60s_walkforward_v3.py" "%SERVICEDIR%\imacd_research_audit_v1.py" "%SERVICEDIR%\run_imacd_audit_v1.py" "%SERVICEDIR%\v4r_imacd_filter_audit_v1.py" "%SERVICEDIR%\v4r_meta_label_audit_v1.py" "%SERVICEDIR%\v4r_meta_label_audit_v2.py" "%SERVICEDIR%\v4r_cross_sectional_rank_audit_v1.py" "%SERVICEDIR%\v4r_direction_asymmetry_audit_v1.py" "%SERVICEDIR%\opening_core_audit_v1.py"
...
if "%FRESHV3%"=="1" (
  echo [3/9] Reusing the fresh V3 Tick dataset already on this PC.
) else (
  echo [3/9] No fresh V3 dataset found; running 10-session QMT Tick replay first...
...
echo [4/9] Running fixed iMACD predictor audit for reference...
set "ASTOCK_SKIP_RESEARCH_CHAIN=1"
"%PYEXE%" -u -m services.run_imacd_audit_v1
set "ASTOCK_SKIP_RESEARCH_CHAIN="
if errorlevel 1 goto :runfail

echo [5/9] Running V4R x iMACD validation-only regime-filter audit...
...
echo [6/9] Running V4R rolling-OOF meta-label audit V2...
...
echo [7/9] Running fixed-universe cross-sectional ranking audit...
...
echo [8/9] Running validation-only direction-asymmetry audit...
echo V4R direction remains frozen; validation may enable BOTH, UP_ONLY, or DOWN_ONLY.
echo Frozen TEST is evaluated once and production remains untouched.
"%PYEXE%" -u -m services.v4r_direction_asymmetry_audit_v1
if errorlevel 1 goto :runfail

echo [9/9] Running dedicated 09:30-10:30 opening-core model audit...
echo Train and validation are restricted to opening-core; TEST is touched once.
echo Coverage is still measured versus the full frozen test day so tiny toy coverage cannot pass.
"%PYEXE%" -u -m services.opening_core_audit_v1
set "RC=%ERRORLEVEL%"
...
if "%RC%"=="0" (
  ...
  echo [ERROR] Opening-core audit failed with rc=%RC%.
