...
echo [5/6] Restarting only the L2 training recorder...
(
  ...
  echo   If InStr(cmd, "qmt_l2_training_recorder"^) ^> 0 Or InStr(cmd, "astockl2trainingwatchdog.cmd"^) ^> 0 Then
  echo     p.Terminate
  echo   End If
  echo Next
) > "%KILLVBS%"
...
if errorlevel 1 (
  echo [WARN] Installed; marker not visible yet. Keep QMT logged in.
) else (
  echo [PASS] L2 training recorder active.
)
