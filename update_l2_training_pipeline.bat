...
echo ===============================================
echo A-Stock Level-2 Trainable 60s Pipeline V5
echo Market context + L2 freshness + honest ML test
 echo ===============================================
...
set "REC=%SERVICEDIR%\qmt_l2_training_recorder_v5.py"
set "REC4=%SERVICEDIR%\qmt_l2_training_recorder_v4.py"
set "REC3=%SERVICEDIR%\qmt_l2_training_recorder_v3.py"
set "RECBASE=%SERVICEDIR%\qmt_l2_training_recorder_v2.py"
set "TRAIN=%SERVICEDIR%\train_l2_60s_model_v4.py"
set "TRAIN3=%SERVICEDIR%\train_l2_60s_model_v3.py"
...
echo [1/7] Downloading V5 recorder and V4 trainer...
curl.exe -L --fail --retry 3 -o "%TEMP%\l2recv5.py" "%BASE%/services/qmt_l2_training_recorder_v5.py" || goto :fail
...
curl.exe -L --fail --retry 3 -o "%TEMP%\l2train4.py" "%BASE%/services/train_l2_60s_model_v4.py" || goto :fail
...
findstr /C:"l2-training-recorder-v5-market-freshness-20260902" "%TEMP%\l2recv5.py" >nul || goto :fail
findstr /C:"l2-training-recorder-v4-efficient-20260902" "%TEMP%\l2recv4.py" >nul || goto :fail
findstr /C:"l2-60s-trainer-v4-validation-cost-stability-20260902" "%TEMP%\l2train4.py" >nul || goto :fail
...
"%PYEXE%" -m py_compile "%TEMP%\l2recv5.py" "%TEMP%\l2recv4.py" "%TEMP%\l2recv3.py" "%TEMP%\l2recv2.py" "%TEMP%\l2train4.py" "%TEMP%\l2train3.py" "%TEMP%\l2auto.py" "%TEMP%\l2status.py" "%TEMP%\dir18.py" "%TEMP%\l2mod.py" "%TEMP%\qmtlive.py"
...
echo [4/7] Installing...
copy /Y "%TEMP%\l2recv5.py" "%REC%" >nul || goto :fail
copy /Y "%TEMP%\l2recv4.py" "%REC4%" >nul || goto :fail
copy /Y "%TEMP%\l2recv3.py" "%REC3%" >nul || goto :fail
copy /Y "%TEMP%\l2recv2.py" "%RECBASE%" >nul || goto :fail
copy /Y "%TEMP%\l2train4.py" "%TRAIN%" >nul || goto :fail
copy /Y "%TEMP%\l2train3.py" "%TRAIN3%" >nul || goto :fail
...
findstr /C:"l2-training-recorder-v5-market-freshness-20260902" "%LOGFILE%" >nul 2>&1
if errorlevel 1 (
  echo [WARN] V5 L2 recorder installed; marker not visible yet. Keep QMT logged in.
) else (
  echo [PASS] Market-aware fresh-L2 training recorder active.
)
...
if errorlevel 1 (
  ...
  echo [PASS] Leak-resistant ML auto-trainer active.
)

echo.
echo [OK] Background L2 + ML autopilot V5 configured.
...
echo Adds HS300 and ChiNext L1 relative-strength context with negligible extra load.
echo Core L2 older than 5 seconds is not accepted as true-L2 training data.
echo 5-second samples and +60s smoothed-mid return are preserved.
echo ML actionable labels require movement beyond the recorded noise band and a 2bp execution hurdle.
echo Probability threshold is chosen on validation only; test is never searched for the best threshold.
echo Reports include gross/net edge, calibration, per-symbol pooled stability and explicit readiness failures.
echo Automatic training runs at 11:35 and 15:10 and successful reports sync to Supabase.
